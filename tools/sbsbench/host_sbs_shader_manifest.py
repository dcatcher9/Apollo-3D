...
PREPROCESS = ClosureGroup(
    name="preprocess",
    description="Model-calibration identity closure; production compiles the fused root from the full producer closure.",
    specs=(
...
PARALLAX_V2_PRODUCER = ClosureGroup(
    name="parallax_v2_producer",
    description="Complete authenticated DAV2, OCR, subtitle-locator, and final-coordinate producer.",
    specs=(
        HOST_SBS_NEAR_IDENTICAL_FUSED_PREPROCESS,
        BUFFER_TO_TEX,
        BUFFER_TO_TEX_PAD,
...
PARALLAX_V2_COORDINATE_DIAGNOSTIC = ClosureGroup(
    name="parallax_v2_coordinate_diagnostic",
    description="Dump-only canonical-coordinate producer entry point.",
    specs=(
...
NEAR_IDENTICAL_DETECTOR = ClosureGroup(
    name="near_identical_detector",
    description="Mandatory GPU reuse arbitration, scene seeding, finalization, and reuse-depth closure.",
    specs=(
...
GPU_TRACE = ClosureGroup(
    name="gpu_trace",
    description="Optional diagnostic GPU completion trace.",
    specs=(
        HOST_SBS_GPU_TRACE,
    ),
...
PARALLAX_V2_LIVE_RENDERER = ClosureGroup(
    name="parallax_v2_live_renderer",
    description="Authenticated live signed-parallax renderer and shared fullscreen vertex shader.",
    specs=(
...
PARALLAX_V2_P010_Y = ClosureGroup(
    name="parallax_v2_p010_y",
    description="Optional fail-open P010 luma MRT renderer.",
    specs=(
...
SBS_FLAT_FALLBACK = ClosureGroup(
    name="sbs_flat_fallback",
    description="Independent authenticated flat-identity fallback and shared fullscreen vertex shader.",
    specs=(
        SBS_FLAT_IDENTITY,
        SBS_REPROJECTION_VERTEX,
    ),
...
PARALLAX_V2_LIVE_DIAGNOSTIC = ClosureGroup(
    name="parallax_v2_live_diagnostic",
    description="Dump-only live inverse-map and mask renderers.",
    specs=(
...
CLOSURE_GROUPS: Dict[str, ClosureGroup] = {
    ...
    "parallax_v2_producer": PARALLAX_V2_PRODUCER,
    "parallax_v2_coordinate_diagnostic": PARALLAX_V2_COORDINATE_DIAGNOSTIC,
    "near_identical_detector": NEAR_IDENTICAL_DETECTOR,
    "gpu_trace": GPU_TRACE,
    "parallax_v2_live_renderer": PARALLAX_V2_LIVE_RENDERER,
    "parallax_v2_p010_y": PARALLAX_V2_P010_Y,
    "sbs_flat_fallback": SBS_FLAT_FALLBACK,
    "parallax_v2_live_diagnostic": PARALLAX_V2_LIVE_DIAGNOSTIC,
}
