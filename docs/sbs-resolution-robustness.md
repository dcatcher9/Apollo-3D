...
| V2 candidate and limiter | normalized source-U displacement | The literal requested pop feeds a frame-local 4% source-U container. A vertical shear-2 majorant followed by a slope-0.5 row majorant preserves near disparity while bounding the field independently of encoder resolution. |
| V2 inverse renderer | source UV | Both eyes solve the same bounded final field with opposite signs and a unique 12-step contractive inverse. There is no V1 probe spacing, forward owner, or synthetic fill whose pixel calibration can drift with output size. |
