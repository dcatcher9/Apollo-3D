...
| V2 inverse renderer | source UV + current source-color pixels | Both eyes solve the same bounded final field with opposite signs and a unique 12-step contractive inverse. The experimental positive-correction collar filter expresses its 4/20/6 thresholds and taps in current source-color pixels, never depth-grid or packed-eye pixels. It is depth-grid independent but intentionally not described as stream-resolution invariant. There is no V1 probe spacing, forward owner, or synthetic fill. |
