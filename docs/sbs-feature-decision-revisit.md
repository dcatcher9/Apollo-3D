...
| EMA ordering | **range → pixel** | **range → pixel** | Reverses the old accepted VD3D order. Core gives 4/3 stability wins and no costs; extended is neutral. Fidelity profile retains pixel → range. |
...
| Guided upsample | **off / rejected** | **off / tradeoff** | No depth crush: core/public pre-warp spread ratios are ~1.001/~1.003 and no matched frame falls below 90%; stereo remains ~1.0. The failure is edge correctness: public GT has two boundary regressions plus one warp-halo cost versus one GT win. VD3D core has one halo win and one halo cost. |
...
| Foreground curvature 0.07 | **off / neutral** | **off / rejected** | Apollo is below validated sensitivity. VD3D has a stability regression and no win. |
...
| Scene-cut min/max snap 1.6 | **off / neutral** | **off / neutral** | No validated movement, including the committed hard-cut sequence. |
...
Both profiles retain Bestv2 shift, 432 depth, P2/P98, range EMA 0.18, per-pixel EMA 0.5,
subject tracking/stretch/recenter, and disable guided/plane/curvature/sharpen/snap/range-floor.
