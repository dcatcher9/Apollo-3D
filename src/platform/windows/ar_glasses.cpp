...
namespace ar_glasses {
  ...
  namespace {
    ...
    void expire_remote_pending_locked(std::chrono::steady_clock::time_point now) {
      if (remote_session_pending && !remote_session_pending->handoff_in_progress && !remote_session_pending->setup_in_progress && now >= remote_session_pending->until) {
    ...
    VDISPLAY::display_identity_state_e query_retiring_local_virtual_display(
      ...
      for (const auto &path : paths) {
        ...
        if (matches_retiring_local_virtual_display(retiring, path.targetInfo.adapterId, path.targetInfo.id, target_name.monitorDevicePath, target_name.monitorFriendlyDeviceName)) {
    ...
    bool wait_for_local_virtual_display_retirement_impl(
      ...
      // Never hold ownership_mutex while waiting: remote launch publishes its reservation under
      // that lock before waiting for local retirement. Include lock contention in this budget.
      std::unique_lock retirement_lock(local_virtual_display_retirement_mutex, std::defer_lock);
    ...
    nlohmann::json serialize_topology_recovery(const topology_recovery_t &recovery) {
      ...
      return {
        ...
        {"pending_rect", recovery.pending_rect ? serialize_rect(*recovery.pending_rect) : nlohmann::json(nullptr)},
      };
    }
    ...
    topology_recovery_t parse_topology_recovery(const nlohmann::json &value) {
      ...
      if (recovery.device_path.empty() || (recovery.owned_rects.empty() && !recovery.pending_rect)) {
        throw std::runtime_error("invalid monitor identity or empty recovery transaction");
      }
      return recovery;
    }

    std::vector<topology_recovery_t> parse_topology_recovery_document(
      ...
      recoveries.reserve(entries.size());
    ...
    std::optional<std::vector<topology_recovery_t>> load_topology_recoveries_locked() {
      ...
      try {
        ...
        return parse_topology_recovery_document(value);
    ...
    std::optional<RECT> prepare_topology_recovery_for_mode_change(
      ...
      if (rebase_topology_recovery_for_mode_change(*existing, previous_original_rect, current_rect)) {
    ...
    std::optional<topology_snapshot_t> query_complete_topology_snapshot() {
      ...
      for (std::size_t path_index = 0; path_index < snapshot.paths.size(); ++path_index) {
        ...
        if (path.sourceInfo.modeInfoIdx == DISPLAYCONFIG_PATH_MODE_IDX_INVALID || path.sourceInfo.modeInfoIdx >= snapshot.modes.size() || snapshot.modes[path.sourceInfo.modeInfoIdx].infoType != DISPLAYCONFIG_MODE_INFO_TYPE_SOURCE) {
          return std::nullopt;
        }
    ...
    std::optional<std::vector<std::string>> topology_snapshot_fingerprint(
      ...
      for (const auto &source : snapshot.sources) {
        ...
        if (target_mode_index == DISPLAYCONFIG_PATH_MODE_IDX_INVALID) {
          ...
          if (target_mode_index >= snapshot.modes.size() || snapshot.modes[target_mode_index].infoType != DISPLAYCONFIG_MODE_INFO_TYPE_TARGET) {
            return std::nullopt;
          }
    ...
    std::optional<pre_add_isolation_plan_t> build_pre_add_isolation_plan(
      ...
      if (!physical || !primary || same_source_identity(*physical, *primary) || source_is_cloned(snapshot->sources, *physical)) {
        return std::nullopt;
      }
    ...
    std::optional<target_state_t> isolate_physical_output(
      ...
      for (int attempt = 0; attempt < 4; ++attempt) {
        ...
        if (!same_rect(current_physical, initial_physical_rect) || !same_rect(current_virtual, initial_virtual_rect)) {
          BOOST_LOG(info) << "AR topology changed while its recovery transaction was being persisted; deferring isolation."sv;
          break;
        }
        if (!same_rect(candidate->layout.physical_rect, requested_layout.physical_rect) || !same_rect(candidate->layout.virtual_rect, requested_layout.virtual_rect)) {
      ...
      if (!fresh_plan || !same_topology_snapshot(validated_baseline, fresh_plan->snapshot) || !same_rect(fresh_plan->layout.virtual_rect, requested_layout.virtual_rect) || !same_rect(fresh_plan->layout.physical_rect, requested_layout.physical_rect)) {
      ...
      for (int attempt = 0; attempt < 10; ++attempt) {
        if (auto verified = query_verified_isolation(virtual_identity, target_device_path, requested_layout)) {
    ...
    bool restore_physical_output_position(
      ...
      if (fresh_physical == fresh_snapshot->sources.end() || source_is_cloned(fresh_snapshot->sources, *fresh_physical)) {
        return false;
      }
    ...
    bool wait_for_virtual_display_mode(
      ...
      while (!cancelled() && std::chrono::steady_clock::now() < deadline) {
        ...
        for (int sleep_step = 0; sleep_step < 4 && !cancelled(); ++sleep_step) {
          std::this_thread::sleep_for(50ms);
        }
      }
      return false;
    ...
    bool configure_virtual_display_hdr(
      ...
      if (!wait_for_virtual_display_mode(display_name, identity, physical_device_path, false, 5s, cancelled)) {
        if (cancelled()) {
          return false;
        }
      ...
      if (!wait_for_virtual_display_mode(display_name, identity, physical_device_path, true, 15s, cancelled)) {
        if (cancelled()) {
          return false;
        }
        BOOST_LOG(warning) << "Local AR virtual display did not reach a stable HDR state; using color-managed SDR presentation."sv;
        if (!VDISPLAY::setDisplayHDRByName(display_name.c_str(), false) || !wait_for_virtual_display_mode(display_name, identity, physical_device_path, false, 5s, cancelled)) {
    ...
    public:
      local_session_t(gpu_workload::lease_t live_gpu_lease, std::function<bool()> session_requested):
          live_gpu_lease_(std::move(live_gpu_lease)),
          exclusive_(config::video.local_ar_virtual_display_only),
          session_requested_(std::move(session_requested)) {
      }

      void initialize(const target_state_t &target, std::stop_token controller_stop_token) {
        const auto cancelled = [this, controller_stop_token]() {
          return controller_stop_token.stop_requested() || !session_requested_();
        };
        ...
        if (exclusive_) {
          ...
          if (!same_rect(post_add_target->rect, original_target_rect_) && !same_rect(post_add_target->rect, pre_add_plan->layout.physical_rect)) {
        ...
        const bool configured = update_display_topology([&]() -> bool {
          ...
          if (const auto isolated_target = isolate_target()) {
            ...
            BOOST_LOG(warning) << "Could not establish the local AR source/sink topology; "sv
          ...
          // Color changes follow the initial durable placement. Exclusive sessions serialize the
          // complete update with display reconciliation; extended sessions keep their row journal.
          ...
          if (const auto refreshed_target = find_target(target_device_path_)) {
            ...
            if (refreshed_width != expected_width || refreshed_height != expected_height || refreshed_target->refresh_millihz != active_target.refresh_millihz || refreshed_target->mode != active_target.mode) {
          ...
          if (const auto isolated_target = isolate_target()) {
            ...
            BOOST_LOG(warning) << "Could not resolve the AR presentation topology after color-mode setup; "sv
          ...
          if (const auto final_target = find_target(target_device_path_)) {
            ...
            const int expected_width = active_target.rect.right - active_target.rect.left;
            const int expected_height = active_target.rect.bottom - active_target.rect.top;
            if (!physical_adapter_contract_valid(active_target.adapter_id, final_target->adapter_id) || final_width != expected_width || final_height != expected_height || final_target->refresh_millihz != active_target.refresh_millihz || final_target->mode != active_target.mode || final_target->hdr.active != active_target.hdr.active || (virtual_hdr_active && !final_target->hdr.active)) {
        ...
        // The exclusive transaction can renumber or move the sink during its final verification.
        // Publish its current exact identity and coordinates only after that transaction completes.
        if (!refresh_virtual_display_reference()) {
          return;
        }
        const auto final_target = find_target(target_device_path_);
        if (!final_target || !final_presentation_contract_matches(presentation_target, *final_target)) {
          return;
        }
        presentation_target = *final_target;
        BOOST_LOG(info) << "Local AR desktop layout: "sv
                        << (exclusive_ ? "virtual desktop primary, glasses active, other displays disabled; cursor confined."sv : "extended desktop; physical displays remain active."sv);
        if (cancelled()) {
          return;
        }
      ...
      std::optional<target_state_t> reconfigure_target(
        ...
        stop_presenter();
        if (cancelled() || target.device_path != target_device_path_ || !same_luid(target.adapter_id, target_adapter_id_) || target.mode == presentation_mode_e::unsupported) {
          return std::nullopt;
        }

        std::optional<target_state_t> presentation_target;
        const bool configured = update_display_topology([&]() -> bool {
          if (!exclusive_) {
          ...
          if (cancelled() || !refresh_virtual_display_reference()) {
            return false;
          }
          ...
          auto wait_for_source_color = [&](bool expected_hdr) {
            for (int attempt = 0; attempt < 20 && !cancelled(); ++attempt) {
              std::this_thread::sleep_for(50ms);
              if (!refresh_virtual_display_reference()) {
                continue;
              }
          ...
          presentation_target = isolate_target();
          if (!presentation_target || !same_luid(presentation_target->adapter_id, target_adapter_id_) || presentation_target->mode != target.mode || (target.hdr.known && presentation_target->hdr.active != target.hdr.active)) {
        ...
        const auto final_target = find_target(target_device_path_);
        if (!presentation_target || !final_target || !final_presentation_contract_matches(*presentation_target, *final_target)) {
          return std::nullopt;
        }
        presentation_target = *final_target;
        if (cancelled()) {
    ...
    private:
      ...
      std::optional<target_state_t> isolate_target() {
    ...
    private:
      ...
      void progress_inactive_retirement() {
        const auto now = std::chrono::steady_clock::now();
        if (session_ || now < retirement_retry_after_) {
          return;
        }
        {
          std::lock_guard lock(ownership_mutex);
          // Remote handoff already progresses this same barrier. Do not acquire a session or a
          // GPU lease just to complete cleanup while wear state keeps local presentation off.
          if (local_session_present || !retired_local_virtual_display || remote_blocks_local_locked(now)) {
            return;
          }
        }
      ...
      void monitor_target(const std::optional<target_state_t> &target) {
        const auto device_path = target ? target->device_path : std::wstring {};
        if (device_path == wear_device_path_) {
          return;
        }
        // The physical device owns wear authority. Keep this subscription alive when its virtual
        // desktop exits off-head or yields to a remote client, and across 2D/SBS mode changes.
        wear_monitor_.reset();
      ...
      bool wear_allows_session() {
        if (wear_device_path_.empty()) {
          return false;
        }
        const auto sensor = wear_monitor_ ? std::make_optional(wear_monitor_->snapshot()) : std::nullopt;
        // Read remembered authority after the live snapshot: a confirmed event received while
        // Windows blocked display setup must survive a subsequent unknown/unavailable report.
        const auto remembered = wear_monitor_ ? wear_monitor_->last_confirmed_state() : rayneo::wear_state_e::unknown;
        const auto activation = activation_.observe(
          sensor,
          std::chrono::steady_clock::now(),
          remembered
        );
        if (activation != logged_activation_) {
          logged_activation_ = activation;
          switch (activation) {
            case detail::local_session_activation_e::waiting_for_sensor:
              BOOST_LOG(info) << "Local AR is waiting for the glasses' initial wear state before creating a virtual desktop."sv;
              break;
            case detail::local_session_activation_e::connection:
              BOOST_LOG(info) << "No confirmed wear state is available; local AR virtual-desktop activation follows the glasses' display connection."sv;
              break;
            case detail::local_session_activation_e::worn:
              BOOST_LOG(info) << "AR glasses report worn; local AR virtual-desktop activation is on."sv;
              break;
            case detail::local_session_activation_e::off_head:
              BOOST_LOG(info) << "AR glasses report off-head; local AR virtual-desktop activation is off."sv;
              break;
          }
        }
      ...
      void start_session(const target_state_t &target, std::stop_token stop_token) {
        if (stop_token.stop_requested() || !wear_allows_session()) {
          return;
        }
        ...
        auto candidate = std::make_unique<local_session_t>(std::move(*live_gpu_lease), [this]() {
          return wear_allows_session();
        });
        ...
        if (!rejected_for_remote && !construction_stop.stop_requested() && wear_allows_session()) {
          schedule_failure_retry();
        }
      }
      ...
      void apply(const std::optional<target_state_t> &target, std::stop_token stop_token) {
        ...
        if ((!config::video.local_ar_virtual_display_only && target->is_primary) || target->is_cloned) {
        ...
        BOOST_LOG(info) << "Approved AR display ["sv << target->friendly_name << "] detected at "sv
                        << width << 'x' << height << '@'
                        << (target->refresh_millihz / 1000.0) << "; "sv
                        << mode_name(target->mode) << " local presentation is available."sv;
      ...
      void run(std::stop_token stop_token) {
        ...
        while (!stop_token.stop_requested()) {
          ...
          const auto now = std::chrono::steady_clock::now();
          if (wear_device_path_.empty() && observed) {
            monitor_target(observed);
          }
          if (observed != pending) {
            pending = observed;
          ...
          if (session_ && !wear_allows_session()) {
            // Extended-mode recovery may still need a durable sink-mode rebase. Never restart
            // presentation or mutate to another mode while that off-head teardown is pending.
            std::this_thread::sleep_for(topology_poll_interval);
            continue;
          }

          if (!pending && !wear_device_path_.empty()) {
            // Keep the sensor and its last authority across brief DP mode-switch gaps, including
            // when off-head teardown has already removed the virtual desktop.
            if (now - pending_since >= incompatible_transition_grace) {
              apply(std::nullopt, stop_token);
            }
            std::this_thread::sleep_for(topology_poll_interval);
            continue;
          }
          ...
          if (pending != applied_ && now - pending_since >= topology_debounce) {
            if (can_re_isolate_local_session(pending, applied_, transition_presenter_paused_)) {
              ...
              const auto reconfigured = session_->reconfigure_target(
                *pending,
                stop_token
              );
              if (!reconfigured) {
            ...
            } else if (!session_ || !incompatible_transition_started_ || now - *incompatible_transition_started_ >= incompatible_transition_grace) {
          ...
          } else if (pending == applied_ && transition_presenter_paused_ && session_ && pending && now - pending_since >= topology_debounce && now >= retry_after_) {
          ...
          } else if (pending == applied_ && session_ && !session_stability_confirmed_ && session_->stable()) {
            ...
            reset_failure_backoff();
          } else if (pending == applied_ && session_ && !session_->running() && session_->failed() && now >= retry_after_ && now >= presenter_retry_after_) {
            transition_presenter_paused_ = true;
            session_->pause_presenter(pending);
            if (session_->virtual_display_authoritatively_absent()) {
      ...
      std::wstring wear_device_path_;
      std::unique_ptr<rayneo::wear_monitor_t> wear_monitor_;
      detail::local_session_activation_t activation_;
      std::optional<detail::local_session_activation_e> logged_activation_;
      std::jthread worker_;
    };
  }  // namespace
  ...
  detail::topology_recovery_parse_result_t detail::parse_topology_recovery_json_for_test(
    ...
    try {
      auto recoveries = parse_topology_recovery_document(nlohmann::json::parse(contents));
      return {
        .valid = true,
        .record_count = recoveries.size(),
        .normalized_json = serialize_topology_recovery_document(recoveries).dump(),
      };
  ...
  bool detail::topology_recovery_should_restore_for_test(
    ...
    try {
      const auto recoveries = parse_topology_recovery_document(nlohmann::json::parse(contents));
      return recoveries.size() == 1 &&
             topology_recovery_should_restore(recoveries.front(), current_rect);
    } catch (const std::exception &) {
      return false;
    }
  }
  ...
  std::optional<std::string> detail::rebase_topology_recovery_json_for_test(
    ...
    try {
      auto recoveries = parse_topology_recovery_document(nlohmann::json::parse(contents));
      if (recoveries.size() != 1 || !rebase_topology_recovery_for_mode_change(recoveries.front(), previous_original_rect, current_rect)) {
        return std::nullopt;
      }
      return serialize_topology_recovery_document(recoveries).dump();
    } catch (const std::exception &) {
      return std::nullopt;
    }
  }
  ...
  bool try_claim_local_virtual_display(const std::stop_source &construction_stop) {
    std::lock_guard lock(ownership_mutex);
    if (construction_stop.stop_requested() || local_session_present || retired_local_virtual_display || remote_blocks_local_locked(std::chrono::steady_clock::now())) {
      return false;
    }
    local_session_present = true;
  ...
  bool remote_virtual_display_starting(
    ...
    if (ownership_changed.wait_until(lock, release_deadline, []() {
      ...
      if (!topology_recovery_complete) {
        ...
        ownership_changed.notify_all();
        BOOST_LOG(error) << (retirement_complete ? "The retired local AR display topology could not be recovered; refusing remote virtual-display creation."sv : "The retired local AR virtual display remained in Windows topology; refusing overlapping remote display creation."sv);
        return false;
      }
