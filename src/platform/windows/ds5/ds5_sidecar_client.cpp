...
namespace platf::ds5 {
  ...
  namespace {
    ...
    std::atomic_bool trusted_audio_available {false};
    ...
    constexpr DWORD write_stall_timeout_ms = 250;

    bool transfer_exact(HANDLE pipe, HANDLE stop_event, void *buffer, std::size_t size, bool write, DWORD wait_timeout = INFINITE) {
      std::size_t offset = 0;
      while (offset < size) {
    ...
    bool read_exact(HANDLE pipe, HANDLE stop_event, std::span<std::uint8_t> destination, DWORD timeout = INFINITE) {
      return transfer_exact(pipe, stop_event, destination.data(), destination.size(), false, timeout);
    }
    ...
    std::optional<std::filesystem::path> trusted_sidecar_path() {
      ...
      const auto candidate = std::filesystem::weakly_canonical(configured, path_error);
      if (path_error || candidate.parent_path() != expected_active_root || candidate.filename() != "Sunshine.Ds5Sidecar.exe" || !std::filesystem::is_regular_file(candidate, path_error) || path_error) {
      ...
      if (manifest.get<std::uint32_t>("protocol", 0) != VERSION || manifest_file != "Sunshine.Ds5Sidecar.exe" || expected_digest.size() != 64) {
      ...
      // A framework-dependent apphost loads adjacent DLLs. Validate its entire
      // runtime payload, not just the small executable that locates the DLL.
      const auto files = manifest.get_child_optional("files");
      if (!files || files->empty()) {
        return std::nullopt;
      }
      for (const auto &[name, entry] : *files) {
        const auto relative = std::filesystem::path(name);
        const auto file = std::filesystem::weakly_canonical(expected_active_root / relative, path_error);
        if (path_error || relative.is_absolute() || relative.has_parent_path() || file.parent_path() != expected_active_root) {
          return std::nullopt;
        }
        const auto digest = sha256_file(file);
        if (!digest || *digest != entry.get_value<std::string>()) {
          return std::nullopt;
        }
      }
      for (const auto &entry : std::filesystem::directory_iterator(expected_active_root)) {
        auto extension = entry.path().extension().string();
        std::ranges::transform(extension, extension.begin(), [](unsigned char value) {
          return static_cast<char>(std::tolower(value));
        });
        if ((extension == ".dll" || extension == ".exe" || (extension == ".json" && entry.path().filename() != "component.json")) && !files->count(entry.path().filename().string())) {
          return std::nullopt;
        }
      }
      return candidate;
    ...
    unsigned int probe_runtime(const std::filesystem::path &executable) {
#ifdef SUNSHINE_DS5_SIDECAR_TEST_HOOK
      return 3;
#else
      auto command = L"\"" + executable.wstring() + L"\" --probe-host";
      std::vector<wchar_t> mutable_command(command.begin(), command.end());
      mutable_command.push_back(L'\0');
      STARTUPINFOW startup {sizeof(startup)};
      PROCESS_INFORMATION process_info {};
      if (!CreateProcessW(executable.c_str(), mutable_command.data(), nullptr, nullptr, FALSE, CREATE_NO_WINDOW, nullptr, executable.parent_path().c_str(), &startup, &process_info)) {
        return 0;
      }
      CloseHandle(process_info.hThread);
      DWORD capabilities = 0;
      if (WaitForSingleObject(process_info.hProcess, 10000) == WAIT_OBJECT_0) {
        // A normal process error (especially exit code 1) is never readiness.
        // The probe uses a tagged result so older helpers fail closed too.
        if (!GetExitCodeProcess(process_info.hProcess, &capabilities) || capabilities < 0x41 || capabilities > 0x43) {
          capabilities = 0;
        } else {
          capabilities &= 3;
        }
  ...
  struct sidecar_client_t::impl_t {
    ...
    feedback_queue_t haptics_queue;
    ...
    bool receive(message_t &message, DWORD timeout = INFINITE) {
      std::array<std::uint8_t, HEADER_SIZE> header {};
      if (!read_exact(pipe, stop_event, header, timeout) || read_u32(header.data()) != MAGIC || read_u16(header.data() + 4) != VERSION) {
        return false;
      }
      ...
      return read_exact(pipe, stop_event, message.payload, timeout);
    }
    ...
    void dispatch(message_t &message) {
      ...
      switch (message.type) {
        case message_e::rumble:
          if (p.size() == 6 && p[0] == owned_index && p[1] == client_index && config::input.forward_rumble) {
            feedback_queue->raise(gamepad_feedback_msg_t::make_rumble(p[1], read_u16(p.data() + 2), read_u16(p.data() + 4)));
          }
          break;
        case message_e::adaptive_triggers:
          if (p.size() == 26 && p[0] == owned_index && p[1] == client_index) {
            std::array<std::uint8_t, 10> left, right;
            std::copy_n(p.data() + 6, 10, left.begin());
            std::copy_n(p.data() + 16, 10, right.begin());
            feedback_queue->raise(gamepad_feedback_msg_t::make_adaptive_triggers(p[1], p[2], p[3], p[4], left, right));
          }
          break;
        case message_e::led:
          if (p.size() == 5 && p[0] == owned_index && p[1] == client_index) {
            feedback_queue->raise(gamepad_feedback_msg_t::make_rgb_led(p[1], p[2], p[3], p[4]));
          }
          break;
        case message_e::haptics_pcm:
          if (audio_haptics_requested && haptics_queue && p.size() >= 24 && p[0] == owned_index && p[1] == client_index) {
            const auto frames = read_u16(p.data() + 4);
            const auto pcm_size = static_cast<std::size_t>(frames) * 4;
            if ((p[2] & ~7) == 0 && p[3] == 2 && p[6] == 16 && p[7] == 0 && read_u32(p.data() + 20) == 48000 && frames <= 240 && p.size() == 24 + pcm_size) {
              haptics_queue->raise(gamepad_feedback_msg_t::make_ds5_haptics_pcm(p[1], p[2], frames, read_u32(p.data() + 8), read_u64(p.data() + 12), p.data() + 24, pcm_size));
            }
          }
          break;
    ...
    bool transact(message_e request_type, std::span<const std::uint8_t> payload, message_e reply_type, message_t &reply) {
      ...
      while (!stopping) {
        ...
        if (remaining <= 0ms || !receive(message, static_cast<DWORD>(remaining.count()))) {
          return false;
        }
    ...
    bool launch_and_connect() {
      ...
      if (!job || !SetInformationJobObject(job, JobObjectExtendedLimitInformation, &job_limits, sizeof(job_limits))) {
      ...
      if (!CreateProcessW(executable_w.c_str(), mutable_command.data(), nullptr, nullptr, FALSE, CREATE_NO_WINDOW | CREATE_SUSPENDED, nullptr, executable.parent_path().c_str(), &startup, &process_info)) {
      ...
      do {
        ...
        const auto connected_pipe = CreateFileW(pipe_path.c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, nullptr);
        if (connected_pipe != INVALID_HANDLE_VALUE) {
          ULONG server_pid = 0;
          if (!GetNamedPipeServerProcessId(connected_pipe, &server_pid) || server_pid != GetProcessId(process)) {
            CloseHandle(connected_pipe);
            BOOST_LOG(error) << "Rejected a DualSense pipe owned by another process"sv;
            return false;
          }
          std::lock_guard lock(write_mutex);
          pipe = connected_pipe;
    ...
    bool connect_and_attach(const gamepad_id_t &id, bool audio_haptics, bool genshin_compatibility) {
      ...
      if (!transact(message_e::hello, hello, message_e::hello_reply, reply) || reply.payload.size() != 4) {
        return false;
      }
      const auto capabilities = read_u32(reply.payload.data());
      if (!(capabilities & virtual_device_host::protocol::CAP_HID) || !(capabilities & virtual_device_host::protocol::CAP_OUTPUT)) {
        return false;
      }
      if (audio_haptics && (capabilities & (virtual_device_host::protocol::CAP_AUDIO_FOUR_CHANNEL | virtual_device_host::protocol::CAP_AUTHORED_HAPTICS_PCM)) != (virtual_device_host::protocol::CAP_AUDIO_FOUR_CHANNEL | virtual_device_host::protocol::CAP_AUTHORED_HAPTICS_PCM)) {
        BOOST_LOG(error) << "DualSense sidecar cannot capture authored audio haptics"sv;
        return false;
      }
      ...
      if (use_genshin_identity && (capabilities & CAP_GENSHIN_COMPATIBILITY_IDENTITY) == 0) {
      ...
      if (!transact(message_e::attach, attach_payload, message_e::attach_reply, reply) || reply.payload.size() != 8 || reply.payload[0] != attach_payload[0]) {
        return false;
      }
  ...
  bool audio_haptics_available() noexcept {
    return trusted_audio_available.load(std::memory_order_acquire);
  }
  ...
  bool refresh_component_availability() noexcept {
    ...
    static auto last_probe = std::chrono::steady_clock::time_point::min();
    std::lock_guard lock(probe_mutex);
#ifndef SUNSHINE_DS5_SIDECAR_TEST_HOOK
    if (last_probe != std::chrono::steady_clock::time_point::min() && std::chrono::steady_clock::now() - last_probe < 10s) {
    ...
    try {
      const auto executable = trusted_component_path();
      const auto capabilities = executable ? probe_runtime(*executable) : 0;
      available = capabilities != 0;
      audio_available = (capabilities & 2) != 0;
    } catch (...) {
      available = false;
    }
    trusted_component_available.store(available, std::memory_order_release);
    trusted_audio_available.store(audio_available, std::memory_order_release);
  ...
  bool sidecar_client_t::configured() const {
    return config::input.ds5_enabled && refresh_component_availability();
  }
  ...
  int sidecar_client_t::alloc(const gamepad_id_t &id, feedback_queue_t feedback_queue, bool audio_haptics, bool genshin_compatibility, feedback_queue_t haptics_queue) {
    if (!configured() || _impl->global_index >= 0 || id.globalIndex < 0 || id.globalIndex >= MAX_GAMEPADS || id.clientRelativeIndex >= MAX_GAMEPADS || !feedback_queue) {
      return -1;
    }
    _impl->feedback_queue = std::move(feedback_queue);
    _impl->haptics_queue = std::move(haptics_queue);
  ...
  void sidecar_client_t::submit_input(int global_index, const gamepad_state_t &state) {
    if (!owns(global_index) || !_impl->online) {
      return;
    }
  ...
  void sidecar_client_t::submit_touch(const gamepad_touch_t &touch) {
    if (!owns(touch.id.globalIndex) || !_impl->online) {
      return;
    }
    std::array<std::uint8_t, 20> payload {};
  ...
  void sidecar_client_t::submit_motion(const gamepad_motion_t &motion) {
    if (!owns(motion.id.globalIndex) || !_impl->online) {
      return;
    }
  ...
  void sidecar_client_t::submit_battery(const gamepad_battery_t &battery) {
    if (!owns(battery.id.globalIndex) || !_impl->online) {
      return;
    }
    std::array<std::uint8_t, 4> payload {
      static_cast<std::uint8_t>(battery.id.globalIndex),
      battery.state,
      battery.percentage,
      0
    };
