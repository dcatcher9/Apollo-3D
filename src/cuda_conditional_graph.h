...
namespace cuda_conditional_graph {
  ...
  [[nodiscard]] constexpr bool authenticated_request(
    ...
    return request.token_low_cookie == (request.token_low ^ token_low_cookie) &&
           request.token_high_cookie == (request.token_high ^ token_high_cookie) &&
           request.magic == request_magic && request.reserved_0 == 0u &&
           request.reserved_1 == 0u && request.reserved_2 == 0u;
  }
  ...
  [[nodiscard]] constexpr decision_record_t resolve_proposal(
    ...
    const auto resolved = authenticated_reuse_proposal(proposal, request) ?
                            branch_e::reuse :
                            branch_e::infer;
  ...
  [[nodiscard]] constexpr bool authenticated_receipt(
    ...
    return authenticated_request(request) &&
           ...
           receipt.decision_cookie == (receipt.decision ^ decision_cookie) &&
           ...
           receipt.magic == receipt_magic && receipt.reserved == 0u;
  }
   ...
   * both input child graphs. The decision/request CUdeviceptrs are captured by value and must
   * remain mapped and unchanged for the executable's lifetime; rebuild if a D3D interop remap
   * returns a different base pointer. For the fixed DAV2 graph, two TensorRT-owned pageable scalar
