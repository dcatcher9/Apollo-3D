 ...
 * records the same six base V2 coordinate entrypoints through the non-owning GPU executor used
...
namespace sbs_bench {
  ...
  public:
    ...
    /** Dispatch one authenticated raw-depth frame.
     *
     * `expected_source_sha256` keeps the geometry result paired with the renderer's immutable
     * source snapshot. Source pixels and their color interpretation are deliberately absent from
     * this API because the production geometry core does not consume them.
     */
    bool dispatch(
