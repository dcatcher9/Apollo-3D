...
namespace config {
  ...
  struct video_t {
    ...
    struct sbs_t {
      ...
      bool sync_depth = true;  ///< Synchronous depth: infer on the CURRENT frame and wait for it (~2-3 ms for DA-V2 small) so color and depth always match -- eliminates the async motion ghost. Off = async (depth lags color; only needed for heavy models). Bench 2026-07-09: sync passes every gate, raises 5/8 clip scores, and cuts disocclusion flicker (c339 7.6 -> 2.4).
