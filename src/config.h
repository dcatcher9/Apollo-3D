...
namespace config {
  ...
  struct video_t {
    ...
    struct sbs_t {
      ...
      double norm_pct_lo = 2.0;  ///< Robust normalization, low percentile (0-50): normalize against this percentile of the disparity histogram (binned over the raw [min,max] range) instead of the raw frame minimum, so a few outlier pixels can't squeeze the scene's parallax range. 0 = off (raw min). Default 2.0 (VD3D DepthPercentileEMA p2) as of 2026-07-10 -- beat raw min/max on 7/8 bench clips (recovered depth range from DA-V2 near-spike outlier rejection); the min/max reduction still runs (it defines the histogram range).
      double norm_pct_hi = 98.0;  ///< Robust normalization, high percentile (50-100): counterpart of norm_pct_lo for the maximum (a long near-spike tail is the common DA-V2 failure). 100 = off (raw max). Default 98.0 (VD3D p98) as of 2026-07-10. Only bench cost was c747 rim_over (an edge metric the concealment stage owns).
      ...
      double foreground_curvature = 0.0;  ///< VD3D enhance_foreground_curvature: add a rounded bulge to the foreground so the subject reads as VOLUME, not a flat cutout (targets the "nose/face reads flat" residual). Pushes the interior of near regions (depth > 0.60) toward the viewer with a centered elliptical pow(1.35) profile. NOTE: currently blocked on typical content because per-frame min/max pins the subject at the 1.0 ceiling (no headroom) -- see docs/sbs-3d-roadmap.md. 0 = off; VD3D Bestv2 0.07.
      ...
      bool guided_upsample = false;  ///< Color-guided (joint-bilateral) depth upsample: snaps the depth model's soft silhouettes to the frame's color edges at 2x depth res, and applies a foreground bias + bimodal edge snap (so it reshapes depth, not just resolution). Default OFF (2026-07-10) for the VD3D-faithful pipeline, which has no equivalent pass -- edge crispness is meant to come from the warp/concealment stage (docs/sbs-vd3d-parity.md B1). WARNING: with guided off and no concealment ported yet, silhouettes are softer (bench: fast_motion edge_acc 0->24). Re-enable for the sharper Apollo silhouettes until concealment lands.
