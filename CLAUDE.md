...
- **Visual** — the headless frame-fed harness `sunshine --sbs-bench` (implemented in
  ...
  `--depth-short-side`, `--ema`, `--minmax-ema`, and the subject tracking/lock/recenter/stretch/
  plane-lock controls are the supported A/B levers (see the harness README). Run
