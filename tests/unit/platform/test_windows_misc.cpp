...
namespace {
  class WindowsProcessObservationTest: public ::testing::Test {
  protected:
    void SetUp() override {
      ASSERT_TRUE(OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY | TOKEN_DUPLICATE, &token_));
      ASSERT_TRUE(CreateRestrictedToken(token_, DISABLE_MAX_PRIVILEGE, 0, nullptr, 0, nullptr, 0, nullptr, &restricted_));
      const auto user = token_sid(token_);
      ASSERT_TRUE(user);
      user_id_ = *user;
      ASSERT_TRUE(ConvertStringSidToSidW(platf::from_utf8(user_id_).c_str(), &user_sid_));
      ...
      // The fixture never executes program code or creates windows. Retain its original
      // all-access handle for cleanup regardless of the restrictive DACL installed below.
      ASSERT_TRUE(CreateProcessW(executable.c_str(), command.data(), nullptr, nullptr, FALSE,
        CREATE_SUSPENDED | CREATE_NO_WINDOW, nullptr, nullptr, &startup, &fixture_));
      EXPLICIT_ACCESSW existing {};
      existing.grfAccessPermissions = PROCESS_TERMINATE;
      existing.grfAccessMode = GRANT_ACCESS;
      existing.grfInheritance = NO_INHERITANCE;
      existing.Trustee.TrusteeForm = TRUSTEE_IS_SID;
      existing.Trustee.TrusteeType = TRUSTEE_IS_USER;
      existing.Trustee.ptstrName = static_cast<LPWSTR>(user_sid_);
      PACL restrictive_dacl = nullptr;
      ASSERT_EQ(SetEntriesInAclW(1, &existing, nullptr, &restrictive_dacl), ERROR_SUCCESS);
      const auto free_dacl = util::fail_guard([&]() { LocalFree(restrictive_dacl); });
      ASSERT_EQ(SetSecurityInfo(fixture_.hProcess, SE_KERNEL_OBJECT, DACL_SECURITY_INFORMATION,
        nullptr, nullptr, restrictive_dacl, nullptr), ERROR_SUCCESS);
    }

    void TearDown() override {
      if (fixture_.hProcess) {
        TerminateProcess(fixture_.hProcess, ERROR_PROCESS_ABORTED);
        WaitForSingleObject(fixture_.hProcess, 2000);
        CloseHandle(fixture_.hProcess);
      }
      if (fixture_.hThread) CloseHandle(fixture_.hThread);
      if (restricted_) CloseHandle(restricted_);
      if (token_) CloseHandle(token_);
      if (user_sid_) LocalFree(user_sid_);
    }

    HANDLE open_without_privileges(DWORD access, DWORD &error) {
      HANDLE previous = nullptr;
      if (!OpenThreadToken(GetCurrentThread(), TOKEN_QUERY | TOKEN_IMPERSONATE, TRUE, &previous) &&
          GetLastError() != ERROR_NO_TOKEN) {
        ADD_FAILURE() << "Could not preserve prior thread token";
        return nullptr;
      }
      const auto restore_token = util::fail_guard([&]() {
        if (previous) {
          EXPECT_TRUE(SetThreadToken(nullptr, previous));
          CloseHandle(previous);
        } else {
          EXPECT_TRUE(RevertToSelf());
        }
      });
      if (!ImpersonateLoggedOnUser(restricted_)) {
        ADD_FAILURE() << "Could not impersonate fixture observer: " << GetLastError();
        return nullptr;
      }
      // Disabled token privileges prevent SeDebugPrivilege from bypassing the fixture DACL.
      const auto handle = OpenProcess(access, FALSE, fixture_.dwProcessId);
      error = handle ? ERROR_SUCCESS : GetLastError();
      return handle;
    }

    PROCESS_INFORMATION fixture_ {};
    HANDLE token_ = nullptr;
    HANDLE restricted_ = nullptr;
    PSID user_sid_ = nullptr;
    std::string user_id_;
  };
}  // namespace

TEST_F(WindowsProcessObservationTest, GrantAllowsIdentityAndWaitWithoutAddingMutationRights) {
  constexpr DWORD observation = PROCESS_QUERY_LIMITED_INFORMATION | SYNCHRONIZE;
  DWORD error = 0;
  auto handle = open_without_privileges(observation, error);
  if (handle) CloseHandle(handle);
  ASSERT_EQ(handle, nullptr);
  ASSERT_EQ(error, ERROR_ACCESS_DENIED);

  const auto granted = platf::grant_process_observation_to_user(fixture_.hProcess, user_id_);
  ASSERT_FALSE(granted) << granted.message();
  handle = open_without_privileges(observation, error);
  ASSERT_NE(handle, nullptr) << error;
  const auto close_observer = util::fail_guard([&]() { CloseHandle(handle); });
  FILETIME created {}, exited {}, kernel {}, user {};
  FILETIME expected_created {}, expected_exited {}, expected_kernel {}, expected_user {};
  ASSERT_TRUE(GetProcessTimes(handle, &created, &exited, &kernel, &user));
  ASSERT_TRUE(GetProcessTimes(fixture_.hProcess, &expected_created, &expected_exited, &expected_kernel, &expected_user));
  EXPECT_EQ(CompareFileTime(&created, &expected_created), 0);
  EXPECT_EQ(WaitForSingleObject(handle, 0), WAIT_TIMEOUT);

  const auto preexisting = open_without_privileges(PROCESS_TERMINATE, error);
  EXPECT_NE(preexisting, nullptr) << "The preexisting ACE was not preserved";
  if (preexisting) CloseHandle(preexisting);
  for (const DWORD denied : {PROCESS_VM_READ, PROCESS_VM_WRITE, PROCESS_CREATE_THREAD, PROCESS_DUP_HANDLE}) {
    const auto unexpected = open_without_privileges(denied, error);
    EXPECT_EQ(unexpected, nullptr) << "Unexpected access mask " << denied;
    EXPECT_EQ(error, ERROR_ACCESS_DENIED);
    if (unexpected) CloseHandle(unexpected);
  }
}

TEST_F(WindowsProcessObservationTest, NullDaclIsPreservedWithoutReplacingObjectPolicy) {
  ASSERT_EQ(SetSecurityInfo(fixture_.hProcess, SE_KERNEL_OBJECT, DACL_SECURITY_INFORMATION,
    nullptr, nullptr, nullptr, nullptr), ERROR_SUCCESS);
  EXPECT_FALSE(platf::grant_process_observation_to_user(fixture_.hProcess, user_id_));
  PACL dacl = nullptr;
  PSECURITY_DESCRIPTOR descriptor = nullptr;
  ASSERT_EQ(GetSecurityInfo(fixture_.hProcess, SE_KERNEL_OBJECT, DACL_SECURITY_INFORMATION,
    nullptr, nullptr, &dacl, nullptr, &descriptor), ERROR_SUCCESS);
  const auto free_descriptor = util::fail_guard([&]() { LocalFree(descriptor); });
  EXPECT_EQ(dacl, nullptr);
}

TEST_F(WindowsProcessObservationTest, InvalidAndBroadPrincipalsDoNotGrantObservation) {
  EXPECT_TRUE(platf::grant_process_observation_to_user(fixture_.hProcess, "invalid-sid"));
  EXPECT_TRUE(platf::grant_process_observation_to_user(fixture_.hProcess, "S-1-1-0"));
  DWORD error = 0;
  const auto denied = open_without_privileges(PROCESS_QUERY_LIMITED_INFORMATION | SYNCHRONIZE, error);
