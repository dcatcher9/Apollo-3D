...
TEST(WebUiDesign, TaskbarRepairUiMatchesItsOptInNativeDefault) {
  ...
  const auto default_off = section.find("off", default_code);
  const auto default_end = section.find("@endcode", default_off);
  ASSERT_NE(default_label, std::string::npos);
  ASSERT_NE(default_code, std::string::npos);
  ASSERT_NE(default_off, std::string::npos);
  ASSERT_NE(default_end, std::string::npos);
  EXPECT_LT(default_label, default_code);
  EXPECT_LT(default_code, default_off);
