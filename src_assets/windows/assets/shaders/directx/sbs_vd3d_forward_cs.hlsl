...
void main(uint3 id : SV_DispatchThreadID) {
    ...
    float2 uv = (float2(id.xy) + 0.5f) / float2(eye_w, eye_h);
    ...
    int left_x = clamp((int)round((float)id.x + parallax * (float)eye_w), 0, (int)eye_w - 1);
    int right_x = clamp((int)round((float)id.x - parallax * (float)eye_w), 0, (int)eye_w - 1);
