...
// Geometry is blocked for exactly one following valid update unless that update is itself a new
// qualified appearance proposal: neural-depth normalization can remain unsettled after a flash
// even while normal playback motion resumes. A new broad semantic endpoint remains authoritative
// immediately. This asymmetry distinguishes post-cut recovery from startup, where no weak or
// relative cut path is allowed.
